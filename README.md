...
- ✅ 群组:创建/改名/删除,设备可加入多个群组;每台设备有"当前群组",语音只路由给当前群组相同的在线成员
...
1. 编辑 `firmware/src/config.h`,填入 WiFi 名称/密码和服务器 IP
...
3. 连接成功后下拉框显示所在群组,可切换"当前群组"进行监听/通讯;点"改名"可重命名群组
4. 设备加入群组:在 Web 管理台操作(群组管理 → 添加成员)

> 注:安卓 6+ 拿不到稳定 WiFi MAC,客户端用持久化 UUID 作为设备 ID;管理台里可为其改名区分。

## 协议速查(WebSocket)

- 文本帧 JSON:`hello{id,kind}` → `welcome{device,groups,active_group_id}`;`select_group{group_id}`、`list_groups` → `groups{...}`;被禁用收 `rejected{reason}` 后被断开
